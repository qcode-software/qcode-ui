function thSortMenu(oSpan) {
  ...
  function onMouseUp() { 
	...
	descLink = 'Sort&nbsp;Z-A';
      ...
      if ( primarySortCol ) {
	if ( sortType == 'ASC' ) {
	  oMenu.innerHTML = '<a href="' + descURL + '">' + descLink + '</a>';
	} else {
	  oMenu.innerHTML = '<a href="' + ascURL + '">' + ascLink + '</a>';
	}
      } else {
	oMenu.innerHTML = '<div style="margin-bottom:4px;"><a href="' + ascURL + '">' + ascLink + '</a>' + '</div><div>' +  '<a href="' + descURL + '">' + descLink + '</a></div>';
      }
    }
