    ...
    try {
        ...
        if ( e.name === "RangeError" ) {
            Number.prototype.toLocaleString = function(locales, options) {
                var minIntDigs = options.minumumIntegerDigits;
                var minFraDigs = options.minumumFractionDigits;
                var maxFraDigs = options.maximumFractionDigits;
                var minSigFigs = options.minimumSignificantDigits;
                var maxSigFigs = options.maximumSignificantDigits;

                if ( minSigFigs !== undefined || maxSigFigs !== undefined ) {
                    minSigFigs = coalesce(minSigFigs,1);
                    maxSigFigs = coalesce(maxSigFigs,minSigFigs);
                    if ( minSigFigs !== parseInt(minSigFigs)
                       || minSigFigs > 21
                       || minSigFigs < 0 ) {
                        throw new RangeError('minimumSignificantDigits value is out of range');
                    }
                    if ( maxSigFigs !== parseInt(maxSigFigx)
                         || maxSigFigs < minSigFigs
                         || maxSigFigs > 21 ) {
                        throw new RangeError('maximumSignificantDigits value is out of range');
                    }
                    var string = (this.toPrecision(maxSigFigs) * 1).toString();
